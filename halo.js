  ...
  function updateKeranjang() {
    ...
    let total = 0;
     ...
     keranjang.forEach((produk) => {
            ...
            item.textContent = `${produk.nama} - Rp. ${produk.harga}; - Rp. ${produk.jumlah * produk.harga.toFixed(2)}`;
            daftarKeranjang.appendChild(item);
            total += (produk.jumlah * produk.harga)
        });

        totalBelanjaElem.textContent = `Rp. ${total}`;
        sessionStorage.setItem(`keranjangBelanja`, JSON.stringify(keranjang));
        //document.cookie = `keranjangBelanja=${JSON.stringify(daftarKeranjang)}`;
        function checkout() {
